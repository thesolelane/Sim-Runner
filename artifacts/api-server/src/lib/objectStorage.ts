...
export class ObjectStorageService {
  ...
  /**
   * Upload a local video file to object storage.
   * Returns the normalized object path (e.g. /objects/sim-videos/uuid.webm) to store in DB.
   * Deletes the local file after a successful upload.
   */
  async uploadVideoFile(localPath: string): Promise<string> {
    const privateDir = this.getPrivateObjectDir();
    const uuid = randomUUID();

    // Compute bucket and object name from PRIVATE_OBJECT_DIR
    // PRIVATE_OBJECT_DIR format: /<bucketName>/<restPath> or /<bucketName>
    const dirParts = privateDir.replace(/^\//, "").split("/");
    const bucketName = dirParts[0];
    const privatePath = dirParts.slice(1).join("/");
    const objectName = privatePath
      ? `${privatePath}/sim-videos/${uuid}.webm`
      : `sim-videos/${uuid}.webm`;

    const fileBuffer = await readFile(localPath);
    await objectStorageClient
      .bucket(bucketName)
      .file(objectName)
      .save(fileBuffer, { contentType: "video/webm" });

    // Clean up local temp file (best effort)
    unlink(localPath).catch(() => {});

    return `/objects/sim-videos/${uuid}.webm`;
  }
