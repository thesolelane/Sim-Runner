...
export class OwnerSimulator {
  ...
  async signup(page, role, user) {
    try {
      ...
      const joinBtn = page.locator('button, a').filter({ 
        hasText: /join|signup|get started|contractor/i 
      }).first();

      if (await joinBtn.count() === 0) {
        return { success: false, detail: 'No signup button found' };
      }
      ...
      if (await confirmField.count() > 0) {
        await confirmField.fill('SimPass123!');
      }

      await page.click('button[type="submit"], button:has-text("Sign Up"), button:has-text("Create")');

      await page.waitForURL(/.*dashboard|.*home|.*feed|.*onboarding|.*wallet/, { 
        timeout: 15000 
      });
  ...
  async profile(page, role, user) {
    try {
      ...
      if (await onboardingForm.count() === 0 || !await onboardingForm.isVisible()) {
        return { success: true, detail: 'Profile already complete or not required' };
      }
      ...
      if (user.data.trade) {
        ...
        if (await tradeSelect.count() > 0) {
          await tradeSelect.selectOption(user.data.trade);
        }
      }
      ...
      if (user.data.account_type) {
        ...
        if (await typeSelect.count() > 0) {
          await typeSelect.selectOption(user.data.account_type);
        }
      }
  ...
  async findJobs(page, role, user) {
    try {
      ...
      return { 
        success: count > 0, 
        detail: count > 0 ? `${count} jobs found` : 'No jobs found' 
      };
    } catch (e) {
      return { success: false, detail: e.message };
    }
  ...
  async bid(page, role, user) {
    try {
      ...
      if (await jobCards.count() === 0) {
        return { success: false, detail: 'No jobs to bid on' };
      }
      ...
      if (await bidBtn.count() === 0) {
        return { success: false, detail: 'No bid button found' };
      }
      ...
      await page.fill('textarea[name="message"], textarea[placeholder*="message" i]', 
        'Licensed electrician, 8 years experience. Available next week.');
  ...
  async buyCredits(page, role, user) {
    try {
      ...
      if (await creditBtn.count() === 0) {
        return { success: true, detail: 'Credits page accessible (Stripe test mode)' };
      }
  ...
  async postJob(page, role, user) {
    try {
      ...
      await page.fill('textarea[name="description"], textarea[placeholder*="description" i]', 
        'Need full electrical rewire for 5,000 sq ft commercial space.');
  ...
  async reviewBids(page, role, user) {
    try {
      ...
      return { 
        success: true, 
        detail: count > 0 ? `${count} bids to review` : 'No bids yet (expected for new account)' 
      };
    } catch (e) {
      return { success: false, detail: e.message };
    }
  ...
  report() {
    ...
    if (this.screenshots.length > 0) {
      console.log(`  📸 Screenshots: ${this.screenshots.length} failures captured`);
    }
    console.log(`${'═'.repeat(50)}\n`);
